...
def flight(identifier):
    ...
    with closing(connect()) as db:
        ...
        data['evidence']=[dict(r,payload=json.loads(r['payload'])) for r in reversed(db.execute('SELECT id,ts,kind,actor,payload FROM events WHERE subject IN (?,?) ORDER BY id DESC LIMIT 40',(identifier,data.get('task_id') or '')).fetchall())]
    return data
