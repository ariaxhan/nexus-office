...
def activate(body: dict) -> dict:
    ...
    if not all(isinstance(fields[key], str) for key in ('repo', 'path', 'state')) or fields['state'] not in ('open', 'missing', 'closed'):
        raise ValueError('Active document needs repo, path and state open|missing|closed')
    with LOCK:
