...
// client/phone/office-issues.js
init_office_ui();
var STATES = [["", "Any state"], ["untriaged", "Untriaged (no labels)"], ["ready", "Ready"], ["active", "Tower attempting"], ["needs_you", "Needs you"], ["held", "Held"], ["owned", "Owned"], ["in_pr", "In PR"], ["labeled", "Labeled, not queued"]];
var SORTS = [["priority", "Priority"], ["updated", "Recently updated"], ["idle", "Least recently updated"], ["age", "Oldest"], ["repo", "Repository"], ["number", "Issue number"]];
var RANK = { p0: 0, p1: 1, p2: 2 };
var ORDER = { priority: (a, b) => (RANK[a.priority] ?? 3) - (RANK[b.priority] ?? 3) || b.age_days - a.age_days, updated: (a, b) => b.updated_at.localeCompare(a.updated_at), idle: (a, b) => a.updated_at.localeCompare(b.updated_at), age: (a, b) => a.created_at.localeCompare(b.created_at), repo: (a, b) => a.repo.localeCompare(b.repo) || a.number - b.number, number: (a, b) => b.number - a.number };
async function issueInventory(parent, open, fresh = false) {
  const data = await api("/api/github/inventory" + (fresh ? "?fresh=1" : ""));
  parent.append(el("p", "muted", `Observed ${data.generated_at}${data.refreshing ? " \xB7 refreshing in the background" : ""} \xB7 ${data.source}`));
  if (data.registry_error) parent.append(el("p", "error", data.registry_error));
  parent.append(button("Refresh from GitHub", async () => {
    parent.replaceChildren();
    await issueInventory(parent, open, true);
  }));
  const filters = { group: "", repo: "" };
  const groups = el("div", "grid");
  ...
  search2.type = "search";
  const state = field(list, "State", select(STATES, ""));
  const sort = field(list, "Sort", select(SORTS, "priority"));
  const scope = el("p", "muted");
...
function issueCard(item, open) {
  ...
  const facts = [item.state.replace("_", " "), item.priority, item.labels.filter((label) => !(label.toLowerCase() in RANK)).join(", "), `${item.age_days}d old`, `updated ${item.idle_days}d ago`, `${item.comments} comments`].filter(Boolean).join(" \xB7 ");
  return card(`${item.repo}#${item.number} ${item.title}`, facts + tower, () => open(item.repo, item));
}
function issueControls(body, repo, data, act, reload) {
  const labels = (data.labels || []).map((label) => typeof label === "string" ? label : label.name);
  const box = section(body, "Triage");
  const bump = el("div", "stack");
  box.append(bump);
  bumpControls(bump, repo, data.number, act, reload);
  const name = field(box, "Label", el("input"));
  name.placeholder = "label name";
  box.append(button("Add label", async () => {
    await act({ action: "label_add", repo, number: data.number, labels: [name.value.trim()] });
    await reload();
  }));
  for (const label of labels) box.append(button(`Remove ${label}`, async () => {
    await act({ action: "label_remove", repo, number: data.number, labels: [label] });
    await reload();
  }));
...
async function bumpControls(parent, repo, number, act, reload) {
  ...
  const verdict = el("p", "muted", "Checking whether Tower would select this\u2026");
  parent.append(verdict);
  const go = button("Bump into Tower queue", async () => {
    await act({ action: "bump", repo, number, priority: priority.value });
    await reload();
  }, "primary");
  parent.append(go);
  async function check() {
    try {
      const answer = await api(`/api/github/bump-check?repo=${encodeURIComponent(repo)}&number=${number}&priority=${priority.value}`);
      verdict.textContent = answer.eligible ? `Tower would select this: adds ${["ready", priority.value].join(", ")}` : `Bump refused: ${answer.reason}`;
      go.hidden = !answer.eligible;
    } catch (error) {
      verdict.textContent = error.message;
      go.hidden = true;
    }
  }
  priority.addEventListener("change", check);
  await check();
}
function editIssue(repo, data, act, reload) {
  const body = el("div", "stack");
  $("#detail-body").append(body);
  const title = field(body, "Title", el("input"));
  title.value = data.title || "";
  const text = field(body, "Description", el("textarea"));
  text.value = data.body || "";
  body.append(button("Save to GitHub", async () => {
    const change = { action: "edit", repo, number: data.number };
    if (title.value !== data.title) change.title = title.value;
    if (text.value !== (data.body || "")) change.body = text.value;
...
async function find(parent) {
  ...
  const projects = section(parent, "Projects");
  await guarded(projects, () => projectRoster(projects));
  parent.append(card("Every open GitHub issue", "TBS, Matra and Tower: counts, filters and triage", () => {
