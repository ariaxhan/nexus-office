...
function gateDetail(gate) {
  const body = sheet("Needs you");
  body.append(markdownView(gate.question || gate.text || JSON.stringify(gate)));
  for (const [answer, label] of [["allow", "Allow once"], ["deny", "Deny"]]) body.append(button(label, async () => {
    await api("/api/gate", { question_id: gate.id, answer });
