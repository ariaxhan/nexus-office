...
class BuzzTest(unittest.TestCase):
    ...
    def test_the_body_field_is_board(self):
        ...
        with self.relay() as (calls, _):
            self.buzz.notify("gate", "a gate is raised: northwind wants Bash")
        req, timeout = calls[0]
        body = json.loads(req.data.decode("utf-8"))
        self.assertEqual(list(body), ["board"])
        self.assertEqual(body["board"], "a gate is raised: northwind wants Bash")
    ...
    def test_gate_raised_names_the_bot_the_permission_and_the_target(self):
        ...
        self.assertEqual(
            text,
            "a gate is raised: northwind wants Bash on "
...
class WatchTest(unittest.TestCase):
    def test_merge_and_terminal_are_quiet_because_delivery_owns_its_buzz_send(self):
