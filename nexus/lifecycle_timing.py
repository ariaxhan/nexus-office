...
ATTEMPT_KINDS = KINDS - {"lifecycle.recognized"}
EDGES = (
...
def _unique(events):
    ...
    for event in sorted(events, key=lambda e: e["id"]):
        ...
        kind = event.get("kind")
        if kind not in KINDS or not _identity(event) or not key:
            errors.append({"event_id": event.get("id"), "reason": "invalid_contract"})
            continue
        if (event.get("subject") != payload["work_key"] or _stamp(event) is None or
                not payload.get("task_id") or payload.get("source_revision") is None or
                (kind in ATTEMPT_KINDS and not payload.get("attempt_id")) or
                (kind in ("lifecycle.wait_started", "lifecycle.wait_ended") and
                 (not payload.get("wait_id") or payload.get("wait_kind") not in WAIT_KINDS)) or
...
def _work_row(events, verified):
    ...
    stamps, spans, raw_waits, open_waits = {}, [], defaultdict(float), []
    ...
    for selected in chain:
        ...
        if any(sum(e["kind"] == kind and e["payload"].get("attempt_id") == aid for e in scoped) > 1
    ...
    for begin in waits:
        ...
        b = wp.get("eligible_at") if kind == "backoff" and wp.get("eligible_at") is not None else (
            _stamp(ends[0]) if len(ends) == 1 else None)
    ...
    pub = [e for e in scoped if e["kind"] == "lifecycle.office_published" and
           e["payload"].get("verified_event_id") == verified["id"]]
    if len(pub) > 1:
        errors.append("ambiguous_publication")
    stamps["published_at"] = _stamp(pub[0]) if len(pub) == 1 else None
    ...
    if start is None or end < start or errors:
        ...
        envelope = end - start
    outcomes = [e["payload"].get("outcome") for e in scoped if
                e["kind"] == "lifecycle.attempt_finished" and
                e["payload"].get("attempt_id") in attempt_ids]
    costs = [e["payload"].get("worker_wall_s") for e in scoped if
             e["kind"] == "lifecycle.attempt_finished" and
             e["payload"].get("attempt_id") in attempt_ids]
    return dict(work_key=key, source_revision=revision, verified_event_id=verified["id"],
                ...
                failed_attempts=sum(x == "failed" for x in outcomes),
                held_attempts=sum(x in ("HELD", "held") for x in outcomes),
                worker_wall_s=sum(costs) if costs and all(isinstance(x, (int, float)) for x in costs) else None)
...
def project(events, *, as_of, window_s=WINDOW_S):
    ...
    for key, rows in grouped.items():
        ...
        if latest and _revision_key(latest["payload"].get("source_revision")) not in valid_revisions:
            ...
            waits = [e for e in related if e["kind"] == "lifecycle.wait_started"]
            open_waits = []
            for wait in waits:
            ...
            eligible_times = [e["payload"].get("eligible_at") for e in waits if
                              isinstance(e["payload"].get("eligible_at"), (int, float)) and
                              e["payload"]["eligible_at"] <= as_of]
            eligible_at = max(eligible_times, default=None)
            pending_after_eligibility = eligible_at is not None and not any(
                _stamp(e) >= eligible_at for e in attempts)
            backlog.append(dict(work_key=key, source_revision=revision,
                                age_s=as_of - base, age_basis="actionable" if actionable is not None else "recognized",
                                ...
                                open_waits=open_waits, eligible_at=eligible_at,
                                post_eligibility_age_s=as_of - eligible_at if pending_after_eligibility else None))
