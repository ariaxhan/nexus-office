...
def _overview(ledger):
    ...
    issue_flights = _issue_flights(ledger)
    failures = [f for f in [*flights, *issue_flights] if f['state'] == 'failed']
    ...
    health = _health(plan, flights, failures, running, age, _issue_failed(issue_flights))
...
def _issue_flights(ledger):
    return ledger.conn.execute("SELECT f.* FROM flights f JOIN tasks t ON t.id=f.task_id "
                               "WHERE t.dedupe_key LIKE ? ORDER BY f.created_at DESC LIMIT 30",
                               (f'github:{REPO}#%',)).fetchall()


def _issue_failed(flights):
    latest = next((f for f in flights if f['state'] in ('failed', 'produced', 'landed')), None)
    return bool(latest and latest['state'] == 'failed')
...
def _health(plan, flights, failures, running, age, issue_failed=False):
