...
### Resumable GitHub collection (pending release)
- Prior turn deployed 87ce6ec and verified the real selected-line composer journey. A fresh audit found 75/96 repositories indexed: serial full-repository collection held 20 repositories behind one large upstream.
- Collection now rotates after eight provider pages or coherent PR snapshots. A private SQLite journal commits records and the next cursor together. Network failure or restart resumes unfinished work; the complete JSONL snapshot stays authoritative until atomic replacement.
- Publication generation/time survive a crash after replacement but before journal deletion. Cursor cycles fail before checkpoint commit and can recover on retry. In-progress fetched counts are visible. Reading a corrupt header no longer postpones its repair cooldown.
- The collector preserves the existing 5 GiB free-space reserve before provider reads, disk writes and snapshot copying. This guards disk capacity; it does not bound in-memory serialization. Low-space failures retain committed progress.
- Independent reviewer verify_scope approved fairness, preservation and recovery boundaries. All 24 focused tests passed. A real isolated armature-ai collection resumed across ten one-batch calls and published 51 records, including its formerly rejected large PR diff. Only provider GET requests; production cache was untouched by that proof.
- Final parent verification 86461 passed, including Python, Node and Swift suites and the armed complexity gate (72 unchanged, zero regressions/new violations). Physical-phone and provider-action acceptance requirements remain unchanged.


### Real provider acceptance and awake readiness (2026-09-08)
- Release c5c6790 is deployed on both production HTTPS ports. Full parent verification passed: 1291 Python tests plus Node and Swift; complexity baseline unchanged.
- Real Office HTTPS actions passed in private repository ariaxhan/nexus-office-acceptance-20260908: issue creation, a 5890-character body, a 3000-character comment, comment deduplication, labels, close/reopen, exact-head diff and review, stale-head refusal, non-pipeline refusal, successful squash merge, and merge request replay without another effect. Fixture PR #1 merged after real Actions verify run 34217996053 succeeded; negative PR #2 closed. Durable proof: /tmp/office-provider-http-acceptance.json.
- Real lost-confirmation recovery passed with an isolated temporary ledger. After GitHub accepted a fixture comment, the test injected a response failure. Repeating the request performed no additional write; reconciliation found the existing provider comment and confirmed the receipt. Exactly one provider write, zero replay writes. Proof: /tmp/office-provider-reconcile-proof.json; comment 5584121003. Production ledger was not modified by fault injection.
...
### Recovery retry admission (2026-09-08)
- Confirmed a scheduling defect: failed repositories left the fair queue, while refresh_all held its lock until all large upstream work completed. The 300-second retry delay alone could not re-admit recovered sources. A regression failed with one attempt instead of two before the fix.
- Due failed repositories now rejoin the tail after each bounded slice; queued entries are not duplicated, fresh failures retain cooldown, and successful snapshots are excluded. Empty queues still release the lock without sleeps or extra workers. Independent review approved fairness and termination; all 44 focused GitHub tests passed.
- Real source recovery from the preceding turn: Vaults324 records fully indexed/searchable, including PR87; heycontent-web PR1 large diff now reads from a clean verified-origin local checkout. Exact current-owner proofs: /tmp/office-vaults-search-owner-proof.json and /tmp/office-heycontent-history-recovery-proof.json. Complete backfill and physical-phone acceptance remain open.
- Full parent54281 passed:1298 Python tests plus Node/Swift; complexity72 unchanged, zero regressions/additions. Red/green receipts retained in /tmp/office-retry-admission-{red,green,parent}.log.
