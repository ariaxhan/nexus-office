...
comment generation failures, not evidence of a new implementation pass. The retained
`issue-pipeline/.runtime/receipts.jsonl` records the decide lane starting and ending
with `decided: fallback` for IA #3 (21:51:50–21:53:54 UTC), #5 (00:00:14–00:02:40),
modelmind #108 (00:18:11–00:20:11), and IA #8 (00:53:50–00:55:34). The other three
generic comments have no matching decide-lane receipt in that file. Their earlier
substantive issue comments are the available work and failure receipts; no matching
`github-work` tasks or flights exist in the current Tower ledger. The generator of
those old GitHub comments is not in this checkout. Office's `client/office-sync.py` previously parsed their
